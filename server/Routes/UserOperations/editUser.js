...
router.post("/EdytujUzytkownika", async (req, res) => {
    ...
    connection.query("SELECT * FROM uzytkownicy WHERE Login = ? AND ID_Uzytkownika != ?", [req.body.Login, req.body.UserID], (err, results) => {
        if (err) {
            console.error(err);
    ...
    */
    ...
    connection.query("SELECT * FROM uzytkownicy WHERE Telefon = ? AND ID_Uzytkownika != ?", [req.body.Phone, req.body.UserID], (err, phoneResults) => {
        if (err) {
            console.error(err);
    ...
    console.log("Kod:"+Zipcode)
    console.log("Telefon:"+Phone)
    console.log("Login:"+Login)
    // console.log("Haslo:"+Password)

    const sql = "UPDATE uzytkownicy SET Imie = ?, Nazwisko = ?, Adres = ?, Kod_Pocztowy = ?, Telefon = ?, Login = ? WHERE ID_Uzytkownika = ?";

    connection.query(sql, [Name, Surname, Adress, Zipcode, Phone, Login, UserID], (err, result) => {
        if (err) {
            console.error(err);
            return res.status(500).send("Błąd podczas aktualizacji użytkownika");
        }
    
        const message = "Zaktualizowano użytkownika o ID: " + UserID + "i loginie:" + Login;
        res.send(message);
    });
})
})
